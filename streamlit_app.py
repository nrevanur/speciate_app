...
class StreamlitApp:
    def __init__(self):        
        self.user_email = st.query_params.get("user_email")
        self.stream_response_flag = True
        self.connection_url = f"{Config.CHAT_URL}?user_email={self.user_email}&stream_response={self.stream_response_flag}"
    ...
    def display_config(self):
        ...
        if response.ok:
            ...
            if self.user_email:
                container.write(f"Reading :blue[{str(config['LLM.CONTEXT_LOG_COUNT'])}] most recent logs for :blue[{self.user_email}]")
            else:
